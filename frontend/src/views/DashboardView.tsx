            ...
            <table class={tableClass}>
              ...
              <tbody>
                {campaigns.value.map((campaign) => (
                  <tr class={rowClass} key={campaign.name}>
...
const statusBadge = (status: CampaignStats["status"]) => {
  const color =
    status === "Active" ? "#0f5132" : status === "Paused" ? "#7a2e0b" : "#1e3a8a";
  const background =
    status === "Active" ? "#d1fae5" : status === "Paused" ? "#ffedd5" : "#dbeafe";
